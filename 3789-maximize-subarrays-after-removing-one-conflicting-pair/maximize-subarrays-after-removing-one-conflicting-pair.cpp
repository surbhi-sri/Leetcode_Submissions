...
public:
    long long maxSubarrays(int n, vector<vector<int>>& conflictingPairs) {
        ...
        // extra[i] = number of extra subarrays by removing the conflicting
        // point i
        ...
        for (int end = 1; end <= n;
             end++) { // visiting each point and treating them as subarray
                      // ending at this point
            // total subarrays ending at this point 'end'

            // O(P)
            for (int& u : conflictingPoints[end]) { // check all conflicting
                                                    // points of end
                if (u >= maxConflict) {
        ...
        return valid + *max_element(ALL(
                           extra)); // you could also write valid +
                                    // *max_element(begin(extra), end(extra))
    }
};